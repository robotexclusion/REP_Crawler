...
- `python main.py` runs the user agent and performs the we crawl
...
- `python query.py` contains modular code for constructing a SQLite query on the gathered data
...
2. This project includes a `uv.lock` file, intended to be used with uv to set up a virtual enviroment and resolve dependencies. to utilize this file, you will need to install uv on your system.

3. Run the following command to download the project: ```bash git clone https://github.com/robotexclusion/REP_Crawler```

4. Navigate into the project directory: ```bash cd REP_Crawler```

5. Set up a virtual envrioment with uv: ```bash uv venv rep_crawler``` ```bash source rep_crawler/bin/activate``` ```bash uv sync```

6. Run the main crawler: ```bash python main.py```
